## copied from GitHub 20250323 https://github.com/zwdzwd/sesame/blob/devel/R/cnv.R
## outcommented some of the plotting functions that create a legend and chromosome line graphs.
...
cnv_plot_extra <- function(
  ...
  cband2col['acen'] <- 'blue' # 'red'                # changed to white/blue transparency
  cband2col['gvar'] <- 'white' # cband2col['gvar'] <- cband2col['gpos75']
  cband2col['gneg'] <- 'white'
  cband2col['gpos'] <- 'white'
  cband2col['stalk'] <- 'white'
  cband2col['gpos25'] <- 'white'
  cband2col['gpos50'] <- 'white'
  cband2col['gpos75'] <- 'white'
  cband2col['gpos100'] <- 'white'
  
  band_color <- cband2col[as.character(cband$gieStain)]
  band_loc <- min(-2, min(seg$bin.signals, na.rm=TRUE)) - 0.6
  y_top <- max(seg$bin.signals, na.rm=TRUE) * 1.1
  y_bottom <- min(seg$bin.signals, na.rm=TRUE) * 1.1
  p <- p + ggplot2::geom_rect(ggplot2::aes(                                   #OUTCOMMENTED
    #xmin = xmins, xmax = xmaxs, ymin = band_loc-0.5,  ymax= 1,     #   ymax = band_loc,
    xmin = xmins, xmax = xmaxs, ymin=y_bottom, ymax=y_top, #ymin = band_loc-0.5,  ymax= 1,     #   ymax = band_loc,
    fill=names(band_color)),alpha = 0.2) +
    ggplot2::scale_fill_manual(values=band_color, guide="none")
  
  ## chromosome boundary
  p <- p + ggplot2::geom_vline(xintercept = c(0,seqstart[-1]/totlen, 1),
                               linewidth=0.5, color="black", alpha=I(0.5))
  #p <- p + ggplot2::geom_vline(xintercept = c(0,seqstart[-1]/totlen, 1),
  #    linetype="dotted", alpha=I(0.5))
  #p <- p + ggplot2::geom_segment(ggplot2::aes(                                #OUTCOMMENTED
...
#'
#' @export
visualizeSegmentsNpb <- function(seg, to.plot=NULL, genes.to.label = NULL) {  # CHANGED FUNCTION NAME
  ...
  p <- ggplot2::ggplot() + ggplot2::geom_point(
    ggplot2::aes(bin.coords$bin.x / totlen,
                 bin.signals, color = bin.signals, alpha = I(0.8)),show.legend = FALSE)
  ...
  p <- p + ggplot2::scale_colour_gradient2(
    limits=c(-0.3,0.3), low='#B50429', mid='white', high='#394DBD', ##changed color red to blue coolwarm from matplotlib
  ...
  #ggplot2::ggsave("/content/plot.png",plot=p, units="cm", dpi=300, width=30, height=10) # target = 1941x687 px
  p
}